/*
    Compile unit: C:\BuildAgent\cm4-build631-TSC6\cmbuild\SKU2_Code\src\target\game\simulator\core\motivecurve.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80123818 -> 0x80123920
*/
// Range: 0x80123818 -> 0x80123920
// this: r29
void MotiveCurveSet::LoadFromFile(unsigned int fileID /* r31 */, signed short id /* r30 */) {
    // Local variables
    class AUTOPTR newPoints; // r1+0x8
    int strCnt; // r31
    class MotiveCurve & curve; // r30
    const char * pointsStr; // r4
}


