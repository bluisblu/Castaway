/*
    Compile unit: C:\BuildAgent\cm4-build631-TSC6\cmbuild\SKU2_Code\src\common\base\core\e_nodelist.h
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800232D0 -> 0x8002348C
*/
// Range: 0x800232D0 -> 0x80023324
// this: r30
ENodeList::~ENodeList() {}

// Range: 0x80023324 -> 0x80023364
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x80023394 -> 0x8002341C
// this: r29
void TNodeList::DeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class EParticleEffect * data; // r0
}

// Range: 0x8002341C -> 0x8002342C
unsigned char ENodeList::IsValid() {}

// Range: 0x8002342C -> 0x80023430
class EParticleEffect * TNodeList::GetData() {}

// Range: 0x80023430 -> 0x80023438
unsigned int ENodeList::GetData() {}

// Range: 0x80023438 -> 0x8002345C
class NLIteratorPtrType * ENodeList::Last() {}

// Range: 0x8002345C -> 0x80023460
class NLIteratorPtrType * ENodeList::Tail() {}

// Range: 0x80023460 -> 0x80023464
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x80023464 -> 0x80023488
class NLIteratorPtrType * ENodeList::Next() {}

// Range: 0x80023488 -> 0x8002348C
class NLIteratorPtrType * ENodeList::Head() {}


// Range: 0x80050DA0 -> 0x80050DE0
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x80050DE0 -> 0x80050E28
// this: r30
class TNodeList & TNodeList::operator=(const class TNodeList & src /* r31 */) {}

// Range: 0x80050E28 -> 0x80050E2C
void TNodeList::AddTail() {}

// Range: 0x80050E2C -> 0x80050E30
int ENodeList::GetCount() {}

// Range: 0x80050E30 -> 0x80050E34
class NLIteratorPtrType * TNodeList::AddHead() {}

// Range: 0x80050E34 -> 0x80050E38
class ISimInstance * TNodeList::GetData() {}

// Range: 0x80050E38 -> 0x80050E3C
void TNodeList::Remove() {}

// Range: 0x80050E3C -> 0x80050E40
class NLIteratorPtrType * TNodeList::InsertAfter() {}

// Range: 0x80050E40 -> 0x80050E44
unsigned char ENodeList::IsEmpty() {}

// Range: 0x80050E44 -> 0x80050E48
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x80050E48 -> 0x80050E4C
class NLIteratorPtrType * TNodeList::Search() {}

// Range: 0x80050E4C -> 0x80050E50
class EIWallPart * TNodeList::GetData() {}

// Range: 0x80050E50 -> 0x80050E54
class ERoomWall * TNodeList::GetData() {}

// Range: 0x80050E54 -> 0x80050E94
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x80050E94 -> 0x80050ED4
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x80050ED4 -> 0x80050F14
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x80050F14 -> 0x80050F54
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x80050F54 -> 0x80050FF0
// this: r29
void TNodeList::DeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class HRObjectFloorRecord * data; // r0
}

// Range: 0x80050FF0 -> 0x80050FF4
class HRObjectFloorRecord * TNodeList::GetData() {}

// Range: 0x80050FF4 -> 0x80051090
// this: r29
void TNodeList::DeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class HRObjectRecord * data; // r0
}

// Range: 0x80051090 -> 0x80051094
class HRObjectRecord * TNodeList::GetData() {}

// Range: 0x80051094 -> 0x80051130
// this: r29
void TNodeList::DeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class HRWallRecord * data; // r0
}

// Range: 0x80051130 -> 0x80051134
class HRWallRecord * TNodeList::GetData() {}

// Range: 0x80051134 -> 0x800511D0
// this: r29
void TNodeList::DeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class HRFloorRecord * data; // r0
}

// Range: 0x800511D0 -> 0x800511D4
class HRFloorRecord * TNodeList::GetData() {}


// Range: 0x8008BBB0 -> 0x8008BBB4
class EILight * TNodeList::GetData() {}

// Range: 0x8008BBB4 -> 0x8008BBF4
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8008BBF4 -> 0x8008BC7C
// this: r29
void TNodeList::DeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class EParticleObj * data; // r0
}

// Range: 0x8008BC7C -> 0x8008BC80
class EParticleObj * TNodeList::GetData() {}

// Range: 0x8008BC80 -> 0x8008BC84
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x8008BC84 -> 0x8008BCE0
// this: r30
void TNodeList::Delete(class NLIteratorPtrType * i /* r31 */) {
    // Local variables
    class EParticleObj * data; // r0
}

// Range: 0x8008BCE0 -> 0x8008BCE4
void TNodeList::Remove() {}

// Range: 0x8008BCE4 -> 0x8008BD24
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8008BD24 -> 0x8008BD64
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8008BD94 -> 0x8008BDD4
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8008BDD4 -> 0x8008BDD8
class EInstance * TNodeList::GetData() {}

// Range: 0x8008BDD8 -> 0x8008BDDC
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x8008BDDC -> 0x8008BDE0
void TNodeList::Remove() {}

// Range: 0x8008BDE0 -> 0x8008BDE4
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x8008BDE4 -> 0x8008BDE8
void TNodeList::Remove() {}

// Range: 0x8008BDE8 -> 0x8008BDEC
class NLIteratorPtrType * TNodeList::Search() {}

// Range: 0x8008BDEC -> 0x8008BDF0
class NLIteratorPtrType * TNodeList::AddHead() {}

// Range: 0x8008BDF0 -> 0x8008BDF4
class NLIteratorPtrType * TNodeList::Search() {}

// Range: 0x8008BDF4 -> 0x8008BDF8
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x8008BDF8 -> 0x8008BDFC
void TNodeList::Remove() {}

// Range: 0x8008BDFC -> 0x8008BE00
class NLIteratorPtrType * TNodeList::Search() {}

// Range: 0x8008BE00 -> 0x8008BE04
class EBaseInstance * TNodeList::GetData() {}


// Range: 0x8016ECE8 -> 0x8016ED28
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8016ED28 -> 0x8016ED2C
class EGameState * TNodeList::GetData() {}

// Range: 0x8016ED2C -> 0x8016ED30
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x8016ED30 -> 0x8016EDCC
// this: r29
void TNodeList::DeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class EGameState * data; // r0
}


// Range: 0x801EC704 -> 0x801EC708
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x801EC708 -> 0x801EC70C
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x801EC70C -> 0x801EC710
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x801EC710 -> 0x801EC714
class NLIteratorPtrType * TNodeList::AddTail() {}


// Range: 0x8021F614 -> 0x8021F618
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x8021F618 -> 0x8021F658
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8021F658 -> 0x8021F698
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8021F698 -> 0x8021F728
// this: r29
void TNodeList::SafeDeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class ERoomWall * data; // r0
}

// Range: 0x8021F728 -> 0x8021F72C
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x8021F72C -> 0x8021F76C
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8021F76C -> 0x8021F808
// this: r29
void TNodeList::DeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class ESpriteRender * data; // r0
}

// Range: 0x8021F808 -> 0x8021F80C
class ESpriteRender * TNodeList::GetData() {}

// Range: 0x8021F80C -> 0x8021F810
void TNodeList::Remove() {}

// Range: 0x8021F810 -> 0x8021F814
class NLIteratorPtrType * TNodeList::AddTail() {}

// Range: 0x8021F814 -> 0x8021F854
// this: r30
TNodeList::TNodeList(unsigned char bDeleteData /* r31 */) {}

// Range: 0x8021F854 -> 0x8021F8E4
// this: r29
void TNodeList::SafeDeleteAll() {
    // Local variables
    class NLIteratorPtrType * i; // r31
    class NLIteratorPtrType * last; // r30
    class ISimInstance * data; // r0
}


// Range: 0x8027DBB0 -> 0x8027DBB8
struct EffectEntry * TNodeList::GetData() {}


// Range: 0x80290DD0 -> 0x80290DD8
class EDL * TNodeList::GetData() {}

// Range: 0x80290DD8 -> 0x80290DE0
class ETexture * TNodeList::GetData() {}


// Range: 0x802E0914 -> 0x802E091C
class EFontSize * TNodeList::GetData() {}


