/*
    Compile unit: C:\BuildAgent\cm4-build631-TSC6\cmbuild\SKU2_Code\src\common\EASTL\1.03.00\include\EASTL\internal\fixed_pool.h
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800D3B30 -> 0x800D3CF8
*/
// Range: 0x800D3B30 -> 0x800D3B34
void fixed_node_pool::deallocate() {}

// Range: 0x800D3B34 -> 0x800D3B44
void fixed_pool::deallocate() {}

// Range: 0x800D3B44 -> 0x800D3B88
// this: r31
fixed_node_pool::fixed_node_pool() {}

// Range: 0x800D3B88 -> 0x800D3BF4
// this: r26
fixed_pool::fixed_pool(void * pMemory /* r27 */, unsigned long memorySize /* r28 */, unsigned long nodeSize /* r29 */, unsigned long alignment /* r30 */, unsigned long alignmentOffset /* r31 */) {}

// Range: 0x800D3BF4 -> 0x800D3BFC
fixed_pool_base::fixed_pool_base() {}

// Range: 0x800D3BFC -> 0x800D3C3C
// this: r31
fixed_node_pool::fixed_node_pool() {}

// Range: 0x800D3C3C -> 0x800D3C80
// this: r31
fixed_node_pool::fixed_node_pool() {}

// Range: 0x800D3C80 -> 0x800D3CC0
// this: r31
fixed_node_pool::fixed_node_pool() {}

// Range: 0x800D3CC0 -> 0x800D3CC4
void * fixed_node_pool::allocate() {}

// Range: 0x800D3CC4 -> 0x800D3CE8
void * fixed_pool::allocate() {
    // Local variables
    struct Link * const pLink; // r4
}

// Range: 0x800D3CE8 -> 0x800D3CEC
void * fixed_node_pool::allocate() {}

// Range: 0x800D3CEC -> 0x800D3CF0
void fixed_node_pool::deallocate() {}

// Range: 0x800D3CF0 -> 0x800D3CF4
void * fixed_node_pool::allocate() {}

// Range: 0x800D3CF4 -> 0x800D3CF8
void * fixed_node_pool::allocate() {}


