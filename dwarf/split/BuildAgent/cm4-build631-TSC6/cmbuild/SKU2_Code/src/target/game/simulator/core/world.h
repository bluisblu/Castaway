/*
    Compile unit: C:\BuildAgent\cm4-build631-TSC6\cmbuild\SKU2_Code\src\target\game\simulator\core\world.h
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8001FF08 -> 0x8001FF10
*/
// Range: 0x8001FF08 -> 0x8001FF10
class PassiveInfluenceMap * cFixedWorld::GetPassiveInfluenceMap() {}


// Range: 0x8005162C -> 0x8005167C
unsigned char cFixedWorld::OutOfBounds() {}

// Range: 0x8005167C -> 0x80051684
class InfluenceMap * cFixedWorld::GetInfluenceMap() {}

// Range: 0x80051684 -> 0x800516AC
unsigned char cFixedWorld::GetFlags() {}

// Range: 0x800516AC -> 0x800516F0
unsigned char cFixedWorld::IsBuildable() {
    // Local variables
    unsigned char tileFlags; // r0
}

// Range: 0x800516F0 -> 0x8005172C
unsigned char cFixedWorld::IsPlaceable() {
    // Local variables
    unsigned char tileFlags; // r0
}


// Range: 0x800A6CB8 -> 0x800A6CEC
void cFixedWorld::SetFlags(unsigned char inFlags /* r31 */) {}


// Range: 0x801237E4 -> 0x80123808
unsigned char cFixedWorld::HasRoof() {
    // Local variables
    unsigned char tileFlags; // r0
}


