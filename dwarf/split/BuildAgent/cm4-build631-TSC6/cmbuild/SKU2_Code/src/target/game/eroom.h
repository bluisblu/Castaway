/*
    Compile unit: C:\BuildAgent\cm4-build631-TSC6\cmbuild\SKU2_Code\src\target\game\eroom.h
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8001AF48 -> 0x8001AF68
*/
// Range: 0x8001AF48 -> 0x8001AF60
unsigned char EIWallPart::IsDiagonal() {}

// Range: 0x8001AF60 -> 0x8001AF68
unsigned char EIWallPart::IsWindow() {}


// Range: 0x80059AD0 -> 0x80059B14
// this: r31
unsigned char EIWallPart::IsDown() {}

// Range: 0x80059B14 -> 0x80059B1C
void EIWallPart::SetForceFadeOff() {}

// Range: 0x80059B1C -> 0x80059B30
unsigned char EIWallPart::isDiagonal() {}


// Range: 0x8021F970 -> 0x8021F9A4
void EIFenceWall::operator delete(void * p /* r31 */) {}

// Range: 0x8021F9A4 -> 0x8021F9E4
void * EIFenceWall::operator new(unsigned long size /* r31 */) {}

// Range: 0x8021F9E4 -> 0x8021FA58
// this: r31
ERoomWall::ERoomWall() {
    // References
    // -> struct [anonymous] __vt__9ERoomWall;
}

// Range: 0x8021FAB0 -> 0x8021FAD0
void EIWallPart::SafeDelete() {}

// Range: 0x8021FAD0 -> 0x8021FAF0
void ERoomWall::SafeDelete() {}

// Range: 0x8021FAF0 -> 0x8021FB38
// this: r31
ERoomWallPtr::~ERoomWallPtr() {}

// Range: 0x8021FB38 -> 0x8021FB40
void ERoomWallPtr::SetPtr() {}

// Range: 0x8021FB40 -> 0x8021FB90
// this: r31
void ERoomWallPtr::Delete() {}

// Range: 0x8021FB90 -> 0x8021FB9C
ERoomWallPtr::ERoomWallPtr() {}

// Range: 0x8021FB9C -> 0x8021FBA4
class ERoomWall * ERoomWallPtr::GetPtr() {}

// Range: 0x8021FBA4 -> 0x8021FBE4
// this: r31
WallFadeParms::~WallFadeParms() {}

// Range: 0x8021FBE4 -> 0x8021FBF0
class ETypeInfo * EIWallPart::GetTypeInfo() {
    // References
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FBF0 -> 0x8021FC00
char * EIWallPart::GetTypeName() {
    // References
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FC00 -> 0x8021FC10
unsigned int EIWallPart::GetTypeKey() {
    // References
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FC10 -> 0x8021FC20
unsigned short EIWallPart::GetTypeVersion() {
    // References
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FC20 -> 0x8021FC40
void EIFenceWall::SafeDelete() {}

// Range: 0x8021FC40 -> 0x8021FC4C
class ETypeInfo * EIFenceWall::GetTypeInfo() {
    // References
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FC4C -> 0x8021FC5C
char * EIFenceWall::GetTypeName() {
    // References
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FC5C -> 0x8021FC6C
unsigned int EIFenceWall::GetTypeKey() {
    // References
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FC6C -> 0x8021FC7C
unsigned short EIFenceWall::GetTypeVersion() {
    // References
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FC7C -> 0x8021FC84
unsigned char EIFenceWall::IsOpaque() {}

// Range: 0x8021FC84 -> 0x8021FC88
void EIWallPart::RealizeShaderTuning() {}

// Range: 0x8021FC88 -> 0x8021FCA8
void EFenceWall::SafeDelete() {}

// Range: 0x8021FCA8 -> 0x8021FCB0
unsigned char EFenceWall::IsWallsOpaque() {}

// Range: 0x8021FCB0 -> 0x8021FCBC
unsigned char EFenceWall::CalWallsOpaque() {}

// Range: 0x8021FD14 -> 0x8021FD4C
class ETypeInfo * EIWallPart::RegisterType() {
    // References
    // -> class ETypeInfo m_typeInfo;
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FD4C -> 0x8021FD54
void EIWallPart::Destruct() {}

// Range: 0x8021FD54 -> 0x8021FD88
void EIWallPart::Construct() {}

// Range: 0x8021FD88 -> 0x8021FD90
void * EIWallPart::operator new() {}

// Range: 0x8021FD90 -> 0x8021FDC0
class EIWallPart * EIWallPart::New() {}

// Range: 0x8021FDC0 -> 0x8021FDF8
class ETypeInfo * EIFenceWall::RegisterType() {
    // References
    // -> class ETypeInfo m_typeInfo;
    // -> class ETypeInfo m_typeInfo;
}

// Range: 0x8021FDF8 -> 0x8021FE00
void EIFenceWall::Destruct() {}

// Range: 0x8021FE00 -> 0x8021FE34
void EIFenceWall::Construct() {}

// Range: 0x8021FE34 -> 0x8021FE84
// this: r31
EIFenceWall::EIFenceWall() {
    // References
    // -> int m_nInstances;
    // -> struct [anonymous] __vt__11EIFenceWall;
}

// Range: 0x8021FE84 -> 0x8021FE8C
void * EIFenceWall::operator new() {}

// Range: 0x8021FE8C -> 0x8021FEBC
class EIFenceWall * EIFenceWall::New() {}


